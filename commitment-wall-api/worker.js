...
export default {
  async fetch(request, env) {
    ...
    // GET /commitments — return all entries newest first
    ...
    // POST /commitments — save a new entry
    if (request.method === 'POST' && url.pathname === '/commitments') {
      ...
      const builds = (body.builds || []).filter(b => typeof b === 'string' && b.trim()).map(b => b.trim().slice(0, 120));
      ...
      return json({ id: meta.last_row_id, name, builds, date: now }, 201);
    }
